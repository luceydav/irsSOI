...
get_dict <- function(year) {
  ...
  # Filter needed rows
  d1 <- d[(stringr::str_which(d, "^state:") - 1):(stringr::str_which(d, "obs:") -2)]
  d1 <- d1[stringr::str_detect(d1, "")]
  col_2 <- d1[stringr::str_detect(d1, "^\\s")]
  col_2 <- stringr::str_remove(col_2, "^\\s*")
  col_2 <- col_2[stringr::str_detect(col_2, "^[1-9]")]

  # Build lists of description bullets
  row_list <- list()
  row_lists <- list()
  for (i in seq_along(col_2) ) {
    if ( stringr::str_detect(col_2[i], "^1") ){
      row_list <- append(row_list, list(col_2[i]))
      if (i < length(col_2) - 1) {
        j <- i + 1
        while( !stringr::str_detect(col_2[j], "^1") ) {
  ...
  # Extract names
  names <- d1[stringr::str_detect(d1, "^[a-z]")]
