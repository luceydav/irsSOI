...
clean_soi <- function(irs_raw) {
  ...
  irs[, numdep := fcoalesce(numdep, n6)]
  ...
  states <- unique(irs[, .(zipcode, state)], by = "zipcode")
  irs <- 
  ...
  cols <- names(irs)[str_which(names(irs), "^a\\d{5}")]
  ...
  irs[, total_tax :=
        fcase(as.numeric(year) < 2007, a09200,
              as.numeric(year) >= 2007, a10300)]
  
  # Set keys
  setkeyv(irs, c("year", "zipcode"))
